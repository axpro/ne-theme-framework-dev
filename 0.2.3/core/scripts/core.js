 ...
 * Licensed under MIT (https://github.com/twbs/bootstrap/blob/master/LICENSE)
 */if(window.navigator.userAgent.match(/IEMobile\/10\.0/)){const msViewportStyle=document.createElement('style');msViewportStyle.appendChild(document.createTextNode('@-ms-viewport{width:auto!important}'));document.querySelector('head').appendChild(msViewportStyle);}
...
unmatch:function(){$this.siblings('.tab-content').children().addClass('tab-pane');}});}});}};Drupal.behaviors.timeline={attach:function(context){var $timelineSelector=$('.timeline');$($timelineSelector).once('timeline',function(){var timelineItemSelector='.timeline .timeline__item',timelineItemsCount=$(timelineItemSelector).length,timeLineButton='<button class="btn btn-time-line">'+Drupal.t("Show all timeline")+'</button>';if(timelineItemsCount>4){$timelineSelector.append(timeLineButton);$(timelineItemSelector).each(function(i){if(i>4){$(this).addClass('hidden');}});$('.btn-time-line',this).click(function(e){e.preventDefault();$(this).hide();$(timelineItemSelector).removeClass('hidden'); // Refreshing scrollspy to recalculate the offset.
...
else {$blocks.push($wrapper.find('.listing__item-link > :first-child'));}var i,max;for(i=0,max=$blocks.length;i<max;i++){var $block=$blocks[i].equalHeight();}});}};Drupal.behaviors.europa_collapse={attach:function(context){Drupal.europa.collapsing();}};Drupal.europa.collapsing=function(showText,hideText){if(!showText){showText=Drupal.t("Show");}if(!hideText){hideText=Drupal.t("Hide");}$('button[data-toggle=collapse]').each(function(){var dependentId=$(this).attr('data-target');var toggler=$(dependentId).hasClass('in')?hideText:showText;var arrow=$('.icon',$(this));var fillMe=$('.toggling-text',$(this));fillMe.text(toggler);$(this).click(function(){var up='icon--up';var down='icon--down';var add=arrow.hasClass(down)?up:down;var rem=arrow.hasClass(down)?down:up;toggler=fillMe.text()==hideText?showText:hideText;fillMe.text(toggler);arrow.addClass(add).removeClass(rem);});});};var trackElements=[];var errorEventSent='Piwik, trackEvent was not fired up.'; /**
 ...
 */window.addEventListener("DOMContentLoaded",function(){var elements=document.getElementsByClassName("form-type-select");for(var ind in elements){if(elements.hasOwnProperty(ind)){elements[ind].innerHTML+='<span class="select-list-arrow"></span>';}}},false);
